...
promised for. The stakes are not only administrative: when authorizations
and claims stall, patients wait for care, and when denials pile up,
providers absorb the labor. A pipeline that produces clean claims faster is
patient-facing infrastructure as much as it is back office.
...
Figure~\ref{fig:architecture} shows how the pipeline is actually built.
Responsibilities are separated into six layers so that the boundary between
model-driven and deterministic behaviour is explicit and auditable.

Each agent is declared with a strict structured output type, so the SDK
constrains generation to a schema and the orchestrator never parses free
text: the scribe emits \texttt{EncounterNote}, the coder
\texttt{CodingResult}, and the resolver \texttt{ResolutionDecision}. Tools
are attached per agent rather than globally, which is what makes the
verification guarantee enforceable: only the coder and the resolver hold
\texttt{search\_icd10}, and only the resolver holds
\texttt{request\_prior\_auth}. The scribe holds no tools at all, so it
cannot reach outside the transcript it was given.

Two design decisions in this layering are worth stating because they run
against the common pattern in agent frameworks. First, there are no agent
handoffs: the workflow order is fixed, so control flow is a plain
asynchronous loop that a reader can follow top to bottom, and the agents are
reduced to well-scoped language operations. Second, the claim builder and
the payer are deliberately kept out of the model path entirely. Every model
call is streamed through \texttt{Runner.run\_streamed()}, and the resulting
token, tool-call, and item events are forwarded verbatim over Server-Sent
Events, which is what allows the interface to expose the agent's reasoning
and tool use as it happens rather than after the fact.

\begin{figure*}
  \centering
  \includegraphics[width=0.92\textwidth]{figures/architecture.png}
  \caption{Layered architecture. Only the agents layer contains model calls;
  the claim builder and payer are ordinary code, which is what makes denial
  recovery attributable to the agent rather than to a stochastic adjudicator.
  Structured output types and per-agent tool grants are the enforcement
  mechanism for the coding-verification and no-fabrication guarantees.}
...
Viewed as an AI system rather than a healthcare product, the gaps are
production-engineering gaps first. There is no human-in-the-loop review
gate: an autonomous claim submitter is a research probe, not something
anyone should deploy. Observability is limited to per-run JSON records
where production would demand tracing, log aggregation, and alerting.
There are no regression evaluations wired into continuous integration, so
a prompt or model change could silently degrade acceptance rates. The
resolver is stateless across attempts, which Section 4.3 shows directly
causes one of the two failures, and there are no cost budgets, idempotency
keys, or versioned prompts. On the domain side: the payer is a simulation
with an intentionally small rule set; real adjudication involves
eligibility, bundling edits, timely filing, and payer-specific medical
policies. The fee schedule is a tiny paraphrased subset because CPT is
AMA-copyrighted. The corpus is simulated dialogue, which is what makes an
open, PHI-free demo possible, but it also means no claim about real-world
performance is made. Coding correctness is not clinically validated, and
the system is presented strictly as a research and teaching artifact, not
billing advice or a compliance tool. No real patient data was used at any
point, no model was fine-tuned, and the dataset is not redistributed.
...
work, in engineering order: give the resolver memory of the attempt history,
which the oscillation failure shows is the binding constraint; put a
human-in-the-loop review queue in front of submissions; wire regression
evaluations into continuous integration so acceptance rates are protected
against prompt and model drift; then richer payer policies (bundling edits,
eligibility), real 837/835 transaction formats, and testing whether the
resolution agent's appeal letters survive review by human billing
professionals.
